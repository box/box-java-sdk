...
public class BoxAIIT {
  ...
  public void aiExtract() throws InterruptedException {
    ...
    BoxFile uploadedFile =
        uploadFileToUniqueFolder(
            api,
            "[aiExtract] Test File.txt",
          ...
          () -> {
            BoxAIResponse response =
                BoxAI.extractMetadataFreeform(
                    ...
                    Collections.singletonList(
                        new BoxAIItem(uploadedFile.getID(), BoxAIItem.Type.FILE)),
                    agentExtract);
  ...
  public void aiExtractStructuredWithFields() throws InterruptedException {
    ...
    BoxFile uploadedFile =
        uploadFileToUniqueFolder(
            api,
            "[aiExtractStructuredWithFields] Test File.txt",
      ...
      retry(
          () -> {
            BoxAIExtractStructuredResponse response =
                BoxAI.extractMetadataStructured(
                    api,
                    Collections.singletonList(
                        new BoxAIItem(uploadedFile.getID(), BoxAIItem.Type.FILE)),
                    Arrays.asList(
                        ...
                        new BoxAIExtractField(
                            "float", "Person age", "Age", "age", null, "How old are you?"),
                        new BoxAIExtractField(
                            "multiSelect",
                            "Person hobby",
                            "Hobby",
            ...
            assertThat(
                response.getAnswer().get("dateOfBirth").asString(), is(equalTo("1990-07-04")));
            assertThat(response.getAnswer().get("age").asInt(), is(equalTo(34)));
            assertThat(
                response.getAnswer().get("hobby").asArray().get(0).asString(),
                is(equalTo("guitar")));
  ...
  public void aiExtractStructuredWithMetadataTemplate() throws InterruptedException {
    ...
    BoxFile uploadedFile =
        uploadFileToUniqueFolder(
            api,
            "[aiExtractStructuredWithMetadataTemplate] Test File.txt",
            "My name is John Doe. I was born in 4th July 1990. I am 34 years old. My hobby is guitar.");
    ...
    MetadataTemplate template =
        MetadataTemplate.createMetadataTemplate(
            ...
            false,
            Arrays.asList(
                new MetadataTemplate.Field(
                    Json.parse(
                            "{\"key\":\"firstName\",\"displayName\":\"First name\","
                                + "\"description\":\"Person first name\",\"type\":\"string\"}")
                        .asObject()),
                new MetadataTemplate.Field(
                    Json.parse(
                            "{\"key\":\"lastName\",\"displayName\":\"Last name\","
                                + "\"description\":\"Person last name\",\"type\":\"string\"}")
                        .asObject()),
                new MetadataTemplate.Field(
                    Json.parse(
                            "{\"key\":\"dateOfBirth\",\"displayName\":\"Birth date\","
                                + "\"description\":\"Person date of birth\",\"type\":\"date\"}")
                        .asObject()),
                new MetadataTemplate.Field(
                    Json.parse(
                            "{\"key\":\"age\",\"displayName\":\"Age\","
                                + "\"description\":\"Person age\",\"type\":\"float\"}")
                        .asObject()),
                new MetadataTemplate.Field(
                    Json.parse(
                            "{\"key\":\"hobby\",\"displayName\":\"Hobby\","
                                + "\"description\":\"Person hobby\",\"type\":\"multiSelect\","
                                + " \"options\":[{\"key\": \"guitar\"}, {\"key\": \"books\"}]}")
                        .asObject())));
    ...
    try {
      ...
      retry(
          () -> {
            ...
            assertThat(response.getAnswer().get("lastName").asString(), is(equalTo("Doe")));
            assertThat(
                response.getAnswer().get("dateOfBirth").asString(),
                is(equalTo("1990-07-04T00:00:00Z")));
            assertThat(response.getAnswer().get("age").asInt(), is(equalTo(34)));
            assertThat(
                response.getAnswer().get("hobby").asArray().get(0).asString(),
                is(equalTo("guitar")));
