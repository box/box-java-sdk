...
/**
 * Represents a comment on a file. Comments can be added directly to a file or they can be created
...
public class BoxComment extends BoxResource {
  ...
  public class Info extends BoxResource.Info {
    ...
    /**
     * Sets the comment's message. The message can contain @mentions by using the
     * string @[userid:username] anywhere within the message, where userid and username are the ID
     * and username of the person being mentioned.
     *
    ...
    /**
     * Gets info about the item this comment is attached to. If the comment is a reply, then the
     * item will be another BoxComment. Otherwise, the item will be a {@link BoxFile}.
     *
    ...
    private void updateItemAsFile(JsonObject itemJSON) {
      ...
      if (this.item != null
          && this.item instanceof BoxFile.Info
          && this.item.getID().equals(itemID)) {
    ...
    private void updateItemAsComment(JsonObject itemJSON) {
      String itemID = itemJSON.get("id").asString();
      if (this.item != null
          && this.item instanceof BoxComment.Info
          && this.item.getID().equals(itemID)) {
        this.item.update(itemJSON);
      } else {
