...
public final class LargeFileUpload {
  ...
  public LargeFileUpload() {
    this.executorService =
        (ThreadPoolExecutor) Executors.newFixedThreadPool(LargeFileUpload.DEFAULT_CONNECTIONS);
  ...
  private BoxFileUploadSession.Info createUploadSession(
      BoxAPIConnection boxApi, String folderId, URL url, String fileName, long fileSize) {
  ...
  public BoxFile.Info upload(
    ...
    BoxFileUploadSession.Info session =
        this.createUploadSession(boxApi, folderId, url, fileName, fileSize);
  ...
  private BoxFile.Info uploadHelper(
      BoxFileUploadSession.Info session,
      InputStream stream,
      long fileSize,
      Map<String, String> fileAttributes)
  ...
  private BoxFileUploadSession.Info createUploadSession(
      BoxAPIConnection boxApi, URL url, long fileSize) {
    BoxJSONRequest request = new BoxJSONRequest(boxApi, url, HttpMethod.POST);
      ...
      if (this.executorService.getQueue().size() < maxQueueSize) {
        ...
        this.executorService.execute(
            new LargeFileUploadTask(
                session.getResource(), bytes, offset, partSize, fileSize, parts, partPostion));
  ...
  /**
   * Generates the Base64 encoded SHA-1 hash for content available in the stream. It can be used to
