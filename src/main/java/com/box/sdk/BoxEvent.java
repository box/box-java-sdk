...
public class BoxEvent extends BoxResource {
   ...
   *
   * <p>Note that there is a bug in the enterprise event stream where certain event sources don't
   * correctly map to a BoxResource.Info. In the case where the event source JSON cannot be mapped
   * to a BoxResource.Info, you can use the {@link #getSourceJSON} method to access the raw JSON
   * representation of the event source.
   *
   ...
   *
   * <p>This method can be used to work around bugs in the enterprise events API where some
   * enterprise event sources don't correctly map to a BoxResource.Info. In this case, this method
   * can be used to access the raw JSON directly.
   *
  ...
  /**
   * Gets the type name as String. Every BoxEvent will have typeName, some will have eventType set
   * to specific value and some will have eventType = UNKNOWN.
   *
   ...
   *
   * <p>The fields and data within the returned JSON object will vary depending on the type of the
   * event.
   *
  ...
  /**
   * Gets info about the collaborator who was given access to a folder within the current
   * enterprise.
   *
   * <p>This field is only populated when the event is related to a collaboration that occurred
   * within an enterprise.
   *
   * @return info about the collaborator who was given access to a folder within the current
   *     enterprise.
   */
  ...
  void parseJsonMember(JsonObject.Member member) {
    ...
    switch (memberName) {
      case "source":
        // Parsing the source might fail due to a bug in the enterprise event stream where the API
        // returns
        // JSON that doesn't correctly map to a BoxResource.Info. If this happens, we set the
        // sourceInfo to null
      ...
      case "accessible_by":
        this.accessibleBy =
            (BoxCollaborator.Info) BoxResource.parseInfo(this.getAPI(), value.asObject());
        break;
      case "created_by":
  ...
  public enum EventType {
    ...
    /**
     * Virus found on a file. Event is only received by enterprises that have opted in to be
     * notified. This is an enterprise-only event.
     */
    ...
    private static final Map<String, BoxEvent.EventType> EVENT_TYPE_MAP =
        new HashMap<>(EventType.values().length);
