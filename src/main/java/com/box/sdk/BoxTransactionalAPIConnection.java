 ...
 *
 * <p>This class handles everything for transactional API that isn't already handled by
 * BoxAPIConnection.
 */
public class BoxTransactionalAPIConnection extends BoxAPIConnection {
   ...
   * @return a BoxAPIConnection which can be used to perform transactional requests.
   */
  public static BoxAPIConnection getTransactionConnection(
      String accessToken, String scope, String resource) {
    ...
    try {
      urlParameters =
          String.format(
              "grant_type=%s&subject_token=%s&subject_token_type=%s&scope=%s",
              GRANT_TYPE,
    ...
    try (BoxJSONResponse response = (BoxJSONResponse) request.send()) {
      ...
      BoxTransactionalAPIConnection transactionConnection =
          new BoxTransactionalAPIConnection(fileToken);
   ...
   * @throws UnsupportedOperationException Box Transactional API does not support authentication
   *     with an auth code
   */
  @Override
