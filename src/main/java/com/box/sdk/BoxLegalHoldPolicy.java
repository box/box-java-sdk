...
/**
 * Represents a legal hold policy. Legal Hold Policy information describes the basic characteristics
 * of the Policy, such as name, description, and filter dates.
 *
 * @see <a href="https://developer.box.com/reference/resources/legal-hold-policy/">Box legal
 *     holds</a>
...
public class BoxLegalHoldPolicy extends BoxResource {
   ...
   * @return information about the Legal Hold Policy created.
   */
  public static BoxLegalHoldPolicy.Info create(
    ...
    try (BoxJSONResponse response = request.send()) {
      ...
      BoxLegalHoldPolicy createdPolicy =
          new BoxLegalHoldPolicy(api, responseJSON.get("id").asString());
  ...
  public static BoxLegalHoldPolicy.Info createOngoing(
      BoxAPIConnection api, String name, String description) {
   ...
   * @return the Iterable of Legal Hold Policies in your Enterprise that match the filter
   *     parameters.
   */
  public static Iterable<BoxLegalHoldPolicy.Info> getAll(
    ...
    return new BoxResourceIterable<BoxLegalHoldPolicy.Info>(
        api,
  ...
  public Info getInfo(String... fields) {
    ...
    URL url =
        LEGAL_HOLD_URL_TEMPLATE.buildWithQuery(
  ...
  /**
   * Updates the information about this retention policy with modified locally info. Only
   * policy_name, description and release_notes can be modified.
   *
  ...
  /**
   * Assigns this legal holds policy to the given box resource. Currently only {@link BoxFile},
   * {@link BoxFileVersion}, {@link BoxFolder} and {@link BoxUser} are supported.
   *
  ...
  public BoxLegalHoldAssignment.Info assignTo(BoxResource resource) {
    return BoxLegalHoldAssignment.create(
        this.getAPI(),
        this.getID(),
        BoxResource.getResourceType(resource.getClass()),
        resource.getID());
  }
  ...
  /**
   * Returns iterable containing assignments for this single legal hold policy. Parameters can be
   * used to filter retrieved assignments.
   *
   * @param type filter assignments of this type only. Can be "file_version", "file", "folder",
   *     "user" or null if no type filter is necessary.
  ...
  public Iterable<BoxLegalHoldAssignment.Info> getAssignments(
      String type, String id, int limit, String... fields) {
    QueryStringBuilder builder = new QueryStringBuilder();
    if (type != null) {
    ...
    return new BoxResourceIterable<BoxLegalHoldAssignment.Info>(
        this.getAPI(),
        LEGAL_HOLD_ASSIGNMENTS_URL_TEMPLATE.buildWithQuery(
            this.getAPI().getBaseURL(), builder.toString(), this.getID()),
        limit) {
  ...
  public Iterable<BoxFileVersionLegalHold.Info> getFileVersionHolds(int limit, String... fields) {
    QueryStringBuilder queryString =
        new QueryStringBuilder().appendParam("policy_id", this.getID());
    if (fields.length > 0) {
      queryString.appendParam("fields", fields);
    }
    URL url =
        LIST_OF_FILE_VERSION_HOLDS_URL_TEMPLATE.buildWithQuery(
            getAPI().getBaseURL(), queryString.toString());
  ...
  public class Info extends BoxResource.Info {
    ...
    /**
     * @return boolean indicating whether the policy will continue applying to files based on
     *     events, indefinitely.
     */
