...
public class BoxSignRequestCreateParams {
  ...
  /**
   * Gets the flag indicating if the sender should be taken into the builder flow to prepare the
   * document.
   *
  ...
  /**
   * Sets the flag indicating if the sender should be taken into the builder flow to prepare the
   * document.
   *
   * @param isDocumentPreparationNeeded whether or not sender should be taken into the builder flow
   *     to prepare the document.
   * @return this BoxSignRequestCreateParams object for chaining.
   */
  public BoxSignRequestCreateParams setIsDocumentPreparationNeeded(
  ...
  /**
   * Gets the flag indicating if usage of signatures generated by typing (text) is enabled. Default
   * is true.
   *
  ...
  /**
   * Sets the flag indicating if usage of signatures generated by typing (text) is enabled. Default
   * is true.
   *
  ...
  /**
   * Sets the message to include in sign request email. This is cleaned,but some html tags are
   * allowed. Links included in the message are also converted to actual links in the email. The
   * message may contain the following html tags: a, abbr, acronym, b, blockquote, code, em, i, ul,
   * li, ol, and strong. Be aware that when the text to html ratio is too high, the email may end up
   * in spam filters. Custom styles on these tags are not allowed.
   *
  ...
  /**
   * Gets the flag indicating if remind for signers to sign a document on day 3, 8, 13 and 18 (or
   * less if the document has been digitally signed already) is enabled.
   *
  ...
  /**
   * Sets the flag indicating if remind for signers to sign a document on day 3, 8, 13 and 18 (or
   * less if the document has been digitally signed already) is enabled.
   *
  ...
  /**
   * Sets the list of prefill tags. When a document contains sign related tags in the content, you
   * can prefill them using this prefillTags by referencing the 'id' of the tag as the externalId
  ...
  public void appendParamsAsJson(JsonObject requestJSON) {
    JsonUtils.addIfNotNull(
        requestJSON, "is_document_preparation_needed", this.isDocumentPreparationNeeded);
    JsonUtils.addIfNotNull(
        requestJSON, "are_text_signatures_enabled", this.areTextSignaturesEnabled);
