...
/**
 * Represents a collaboration allowlist between a user and a Box Enterprise. Collaboration Allowlist
 * enables a Box Enterprise(only available if you have Box Governance) to manage a set of approved
 * users that can collaborate with an enterprise.
 *
...
public class BoxCollaborationAllowlistExemptTarget extends BoxResource {
  ...
  /**
   * Constructs a BoxCollaborationAllowlistExemptTarget for a collaboration allowlist with a give
   * ID.
   *
   ...
   */
  public static BoxCollaborationAllowlistExemptTarget.Info create(
      final BoxAPIConnection api, String userID) {
    ...
    JsonObject requestJSON =
        new JsonObject().add("user", new JsonObject().add("type", "user").add("id", userID));

    request.setBody(requestJSON.toString());
    try (BoxJSONResponse response = request.send()) {
      JsonObject responseJSON = Json.parse(response.getJSON()).asObject();
      BoxCollaborationAllowlistExemptTarget userAllowlist =
          new BoxCollaborationAllowlistExemptTarget(api, responseJSON.get("id").asString());
   ...
   */
  public static Iterable<BoxCollaborationAllowlistExemptTarget.Info> getAll(
  ...
  public static Iterable<BoxCollaborationAllowlistExemptTarget.Info> getAll(
    ...
    URL url =
        COLLABORATION_ALLOWLIST_EXEMPT_TARGET_ENTRIES_URL_TEMPLATE.buildWithQuery(
    ...
    return new BoxResourceIterable<BoxCollaborationAllowlistExemptTarget.Info>(api, url, limit) {
      ...
      protected BoxCollaborationAllowlistExemptTarget.Info factory(JsonObject jsonObject) {
        BoxCollaborationAllowlistExemptTarget userAllowlist =
            new BoxCollaborationAllowlistExemptTarget(api, jsonObject.get("id").asString());
  ...
  public BoxCollaborationAllowlistExemptTarget.Info getInfo() {
    URL url =
        COLLABORATION_ALLOWLIST_EXEMPT_TARGET_ENTRY_URL_TEMPLATE.build(
