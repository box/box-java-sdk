...
final class BinaryBodyUtils {
   ...
   * @param listener Listener that will be notified on writing response. Can be null.
   */
  static void writeStreamWithContentLength(
      BoxAPIResponse response, OutputStream output, ProgressListener listener) {
    try {
      InputStream input;
      if (listener != null) {
        input = response.getBody(listener);
  ...
  /**
   * Get the content length from the API response. In some cases, the Content-Length is not provided
   * in the response headers. This could happen when getting the content representation for a
   * compressed data. In that case the API will switch to chunk mode and provide the length in the
  ...
  /**
   * Writes the content of the input stream to the provided output stream, ensuring the exact number
   * of bytes specified by the expected length is written. If the stream ends prematurely an
   * exception is thrown.
   *
  ...
  static void writeStreamTo(InputStream input, OutputStream output, long expectedLength) {
    ...
    if (expectedLength < 0) {
      throw new RuntimeException(
          "Expected content length should not be negative: " + expectedLength);
    }
    try {
      byte[] buffer = new byte[BUFFER_SIZE];
      ...
      if (totalBytesRead != expectedLength) {
        throw new IOException(
            "Stream ended prematurely. Expected "
                + expectedLength
                + " bytes, but read "
                + totalBytesRead
                + " bytes.");
