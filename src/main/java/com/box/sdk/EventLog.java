 ...
 *
 * <p>An EventLog cannot be instantiated directly. Instead, use one of the static methods to
 * retrieve a log of events. Unlike the {@link EventStream} class, EventLog doesn't support
 * retrieving events in real-time.
 */
public class EventLog implements Iterable<BoxEvent> {
  ...
  public static final URLTemplate ENTERPRISE_EVENT_URL_TEMPLATE =
      new URLTemplate("events?stream_type=admin_logs&" + "limit=" + ENTERPRISE_LIMIT);
  ...
  /**
   * Method reads from the `admin-logs` stream and returns {@link BoxEvent} {@link Iterator}. The
   * emphasis for this stream is on completeness over latency, which means that Box will deliver
   * admin events in chronological order and without duplicates, but with higher latency. You can
   * specify start and end time/dates. This method will only work with an API connection for an
   * enterprise admin account or service account with manage enterprise properties. You can specify
   * a date range to limit when events occured, starting from a given position within the event
   * stream, set limit or specify event types that should be filtered. Example:
   *
   * <pre>{@code
   ...
   */
  public static EventLog getEnterpriseEvents(
      BoxAPIConnection api, EnterpriseEventsRequest enterpriseEventsRequest) {
  ...
  /**
   * Method reads from the `admin-logs-streaming` stream and returns {@link BoxEvent} {@link
   * Iterator} The emphasis for this feed is on low latency rather than chronological accuracy,
   * which means that Box may return events more than once and out of chronological order. Events
   * are returned via the API around 12 seconds after they are processed by Box (the 12 seconds
   * buffer ensures that new events are not written after your cursor position). Only two weeks of
   * events are available via this feed, and you cannot set start and end time/dates. This method
   * will only work with an API connection for an enterprise admin account or service account with
   * manage enterprise properties. You can specify a starting from a given position within the event
   * stream, set limit or specify event types that should be filtered. Example:
   *
   * <pre>{@code
  ...
  public static EventLog getEnterpriseEventsStream(
    ...
    return getEnterpriseEventsForStreamType(
        api, enterpriseEventsStreamRequest.getStreamType(), request);
  }
   ...
   *
   * <p>The value returned by this method corresponds to the <code>created_after</code> URL
   * parameter that was used when retrieving the events in this EventLog.
   *
   ...
   *
   * <p>The value returned by this method corresponds to the <code>created_before</code> URL
   * parameter that was used when retrieving the events in this EventLog.
   *
  ...
  /**
   * Gets the maximum number of events that this event log could contain given its start date, end
   * date, and stream position.
   *
   * <p>The value returned by this method corresponds to the <code>limit</code> URL parameter that
   * was used when retrieving the events in this EventLog.
   *
   ...
   *
   * <p>The value returned by this method corresponds to the <code>stream_position</code> URL
   * parameter that was used when retrieving the events in this EventLog.
   *
   ...
   *
   * <p>The value returned by this method corresponds to the <code>next_stream_position</code> field
   * returned by the API's events endpoint.
   *
   ...
   *
   * <p>The chunk size may not be representative of the number of events returned by this EventLog's
   * iterator because the iterator will automatically ignore duplicate events.
   *
   * <p>The value returned by this method corresponds to the <code>chunk_size</code> field returned
   * by the API's events endpoint.
   *
   ...
   *
   * <p>The size is guaranteed to be representative of the number of events returned by this
   * EventLog's iterator.
   *
  ...
  private static final class EventLogRequest {
    ...
    private EventLogRequest(
        Date before, Date after, String position, Integer limit, Collection<String> types) {
      this.before = before;
      this.after = after;
