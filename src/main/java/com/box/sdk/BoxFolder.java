...
/**
 * Represents a folder on Box. This class can be used to iterate through a folder's contents,
 * collaborate a folder with another user or group, and perform other common folder operations
...
public class BoxFolder extends BoxItem implements Iterable<BoxItem.Info> {
  /**
   * An array of all possible folder fields that can be requested when calling {@link
   * #getInfo(String...)}.
   */
  ...
  public static final URLTemplate GET_COLLABORATIONS_URL =
      new URLTemplate("folders/%s/collaborations");
  ...
  public BoxCollaboration.Info collaborate(
      BoxCollaborator collaborator, BoxCollaboration.Role role) {
   ...
   * @param canViewPath the view path collaboration feature is enabled or not. View path
   *     collaborations allow the invitee to see the entire ancestral path to the associated folder.
    ...
    if (sharedLink.getPermissions() != null && sharedLink.getPermissions().getCanEdit()) {
      ...
      sharedLink.setPermissions(
          new BoxSharedLink.Permissions(
              permissions.getCanPreview(), permissions.getCanDownload(), false));
    }
    return sharedLink;
  }
  ...
  public Collection<BoxCollaboration.Info> getCollaborations(String... fields) {
    ...
    URL url =
        GET_COLLABORATIONS_URL.buildWithQuery(
            api.getBaseURL(), queryBuilder.toString(), this.getID());
  ...
  public BoxFolder.Info getInfo(String... fields) {
    ...
    if (fields.length > 0) {
      ...
      url =
          FOLDER_INFO_URL_TEMPLATE.buildWithQuery(
  ...
  public BoxFolder.Info createFolder(String name) {
    ...
    BoxJSONRequest request =
        new BoxJSONRequest(
            this.getAPI(), CREATE_FOLDER_URL.build(this.getAPI().getBaseURL()), "POST");
  ...
  public BoxFile.Info uploadFile(UploadFileCallback callback, String name) {
    FileUploadParams uploadInfo =
        new FileUploadParams().setUploadFileCallback(callback).setName(name);
    return this.uploadFile(uploadInfo);
  }
   ...
   * @return the uploaded file's info.
   */
  public BoxFile.Info uploadFile(
      InputStream fileContent, String name, long fileSize, ProgressListener listener) {
    FileUploadParams uploadInfo =
  ...
  public BoxFile.Info uploadFile(InputStream fileContent, String name, String description) {
    FileUploadParams uploadInfo =
        new FileUploadParams().setContent(fileContent).setName(name).setDescription(description);
    return this.uploadFile(uploadInfo);
  }
  ...
  public BoxWebLink.Info createWebLink(String name, URL linkURL, String description) {
    ...
    BoxJSONRequest request =
        new BoxJSONRequest(
            this.getAPI(), CREATE_WEB_LINK_URL.build(this.getAPI().getBaseURL()), "POST");
  ...
  /**
   * Returns an iterable containing the items in this folder. Iterating over the iterable returned
   * by this method is equivalent to iterating over this BoxFolder directly.
   *
  ...
  /**
   * Returns an iterable containing the items in this folder and specifies which child fields to
   ...
   */
  public Iterable<BoxItem.Info> getChildren(
      String sort, SortDirection direction, final String... fields) {
   ...
   * Returns an iterable containing the items in this folder sorted by name and direction.
   *
   * @param sortParameters describes sorting parameters. Sort parameters are supported only with
   *     offset based pagination. Use {@link SortParameters#none()} to ignore sorting.
  ...
  /**
   * Used to retrieve the watermark for the folder. If the folder does not have a watermark applied
   * to it, a 404 Not Found will be returned by API.
   *
   * @param fields the fields to retrieve.
  ...
  /**
   * Removes a watermark from the folder. If the folder did not have a watermark applied to it, a
   * 404 Not Found will be returned by API.
   */
  ...
  public Metadata createMetadata(String templateName, String scope, Metadata metadata) {
    URL url =
        METADATA_URL_TEMPLATE.buildAlpha(
            this.getAPI().getBaseURL(), this.getID(), scope, templateName);
    BoxJSONRequest request = new BoxJSONRequest(this.getAPI(), url, "POST");
  ...
  private void validateSortIsSelectedWithOffsetPaginationOnly(
      PagingParameters pagingParameters, QueryStringBuilder sortQuery) {
    if (pagingParameters != null
        && pagingParameters.isMarkerBasedPaging()
        && sortQuery.toString().length() > 0) {
  ...
  /**
   * Creates an upload session to create a new file in chunks. This will first verify that the file
   * can be created and then open a session for uploading pieces of the file.
   *
  ...
  public BoxFile.Info uploadLargeFile(
      InputStream inputStream, String fileName, long fileSize, Map<String, String> fileAttributes)
      throws InterruptedException, IOException {
    URL url = UPLOAD_SESSION_URL_TEMPLATE.build(this.getAPI().getBaseUploadURL());
    this.canUpload(fileName, fileSize);
    return new LargeFileUpload()
  ...
  /**
   * Creates a new file using specified number of parallel http connections. Also sets file
   * attributes.
   *
   * @param inputStream the stream instance that contains the data.
   * @param fileName the name of the file to be created.
   ...
   * @return the Iterable of Box Metadata Cascade Policies in your enterprise.
   */
  public Iterable<BoxMetadataCascadePolicy.Info> getMetadataCascadePolicies(
      String enterpriseID, int limit, String... fields) {

    return BoxMetadataCascadePolicy.getAll(
        this.getAPI(), this.getID(), enterpriseID, limit, fields);
  }
  ...
  public BoxFolderLock.Info lock() {
    ...
    BoxJSONRequest request =
        new BoxJSONRequest(
            this.getAPI(), FOLDER_LOCK_URL_TEMPLATE.build(this.getAPI().getBaseURL()), "POST");
    request.setBody(body.toString());
    try (BoxJSONResponse response = request.send()) {
      JsonObject responseJSON = Json.parse(response.getJSON()).asObject();

      BoxFolderLock createdFolderLock =
          new BoxFolderLock(this.getAPI(), responseJSON.get("id").asString());
  ...
  public Iterable<BoxFolderLock.Info> getLocks() {
    ...
    return new BoxResourceIterable<BoxFolderLock.Info>(
        api, FOLDER_LOCK_URL_TEMPLATE.buildWithQuery(api.getBaseURL(), queryString), 100) {
      @Override
      protected BoxFolderLock.Info factory(JsonObject jsonObject) {
        BoxFolderLock folderLock = new BoxFolderLock(api, jsonObject.get("id").asString());
  ...
  public class Info extends BoxItem.Info {
    ...
    /**
     * Gets the metadata on this folder associated with a specified scope and template. Makes an
    ...
    /**
     * Get the field is_externally_owned determining whether this folder is owned by a user outside
     * of the enterprise.
     *
    ...
    /**
     * Returns the flag indicating if collaborators who are not owners of this folder are restricted
     * from viewing other collaborations on this folder.
     *
     * @return boolean flag indicating if collaborators who are not owners of this folder are
     *     restricted from viewing other collaborations on this folder.
     */
    ...
    /**
     * Sets whether collaborators who are not owners of this folder are restricted from viewing
     * other collaborations on this folder.
     *
     * @param canNonOwnersViewCollaborators indicates if collaborators who are not owners of this
     *     folder are restricted from viewing other collaborations on this folderr.
     */
