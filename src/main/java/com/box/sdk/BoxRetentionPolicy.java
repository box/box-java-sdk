...
/**
 * Represents a retention policy. A retention policy blocks permanent deletion of content for a
 * specified amount of time. Admins can create retention policies and then later assign them to
 * specific folders or their entire enterprise.
 *
 * @see <a href="https://developer.box.com/reference/resources/retention-policy/">Box retention
 *     policy</a>
 *     <p>Unless otherwise noted, the methods in this class can throw an unchecked {@link
 *     BoxAPIException} (unchecked meaning that the compiler won't force you to handle it) if an
 *     error occurs. If you wish to implement custom error handling for errors related to the Box
 *     REST API, you should capture this exception explicitly.
 */
...
public class BoxRetentionPolicy extends BoxResource {
  ...
  public static final URLTemplate RETENTION_POLICIES_URL_TEMPLATE =
      new URLTemplate("retention_policies");
  ...
  public static final URLTemplate ASSIGNMENTS_URL_TEMPLATE =
      new URLTemplate("retention_policies/%s/assignments");
  ...
  public static BoxRetentionPolicy.Info createIndefinitePolicy(
    ...
    return createRetentionPolicy(
        api,
        name,
        BoxRetentionPolicyType.Indefinite,
        0,
        BoxRetentionPolicyAction.RemoveRetention,
        optionalParams);
  }
   ...
   * @param api the API connection to be used by the created user.
   * @param name the name of the retention policy.
   * @param length the duration in days that the retention policy will be active for after being
   *     assigned to content.
   * @param action the disposition action can be "permanently_delete" or "remove_retention".
   * @return the created retention policy's info.
   */
  public static BoxRetentionPolicy.Info createFinitePolicy(
      BoxAPIConnection api, String name, int length, BoxRetentionPolicyAction action) {
  ...
  public static BoxRetentionPolicy.Info createFinitePolicy(
    ...
    return createRetentionPolicy(
        api, name, BoxRetentionPolicyType.Finite, length, action, optionalParams);
  }
  ...
  private static BoxRetentionPolicy.Info createRetentionPolicy(
    ...
    try (BoxJSONResponse response = request.send()) {
      ...
      BoxRetentionPolicy createdPolicy =
          new BoxRetentionPolicy(api, responseJSON.get("id").asString());
   ...
   *
   * @param name a name to filter the retention policies by. A trailing partial match search is
   *     performed. Set to null if no name filtering is required.
   * @param type a policy type to filter the retention policies by. Set to null if no type filtering
   *     is required.
   * @param userID a user id to filter the retention policies by. Set to null if no type filtering
   *     is required.
  ...
  public static Iterable<BoxRetentionPolicy.Info> getAll(
    ...
    URL url =
        RETENTION_POLICIES_URL_TEMPLATE.buildWithQuery(api.getBaseURL(), queryString.toString());
   ...
   * @return an iterable containing all folder assignments.
   */
  public Iterable<BoxRetentionPolicyAssignment.Info> getFolderAssignments(
   ...
   * @return an iterable containing all enterprise assignments.
   */
  public Iterable<BoxRetentionPolicyAssignment.Info> getEnterpriseAssignments(
      int limit, String... fields) {
   ...
   * @return an iterable containing all assignments.
   */
  public Iterable<BoxRetentionPolicyAssignment.Info> getAllAssignments(
      int limit, String... fields) {
   ...
   *
   * @param type the type of the retention policy assignment to retrieve. Can either be "folder" or
   *     "enterprise".
   * @param limit the limit of entries per response. The default value is 100.
   * @param fields the fields to retrieve.
   * @return an iterable containing all assignments of given type.
   */
  private Iterable<BoxRetentionPolicyAssignment.Info> getAssignments(
      String type, int limit, String... fields) {
    QueryStringBuilder queryString = new QueryStringBuilder();
    if (type != null) {
    ...
    URL url =
        ASSIGNMENTS_URL_TEMPLATE.buildWithQuery(
            getAPI().getBaseURL(), queryString.toString(), getID());
  ...
  public BoxRetentionPolicyAssignment.Info assignTo(BoxFolder folder) {
    return BoxRetentionPolicyAssignment.createAssignmentToFolder(
        this.getAPI(), this.getID(), folder.getID());
  }
   ...
   */
  public BoxRetentionPolicyAssignment.Info assignToMetadataTemplate(
      String templateID, MetadataFieldFilter... fieldFilters) {
   ...
   *
   * @param templateID the ID of the metadata template to apply to.
   * @param startDateField the date the retention policy assignment begins. This field can be a date
   *     field's metadata attribute key id.
   * @param fieldFilters optional field value filters.
   * @return info about the created assignment.
   */
  public BoxRetentionPolicyAssignment.Info assignToMetadataTemplate(
      String templateID, String startDateField, MetadataFieldFilter... fieldFilters) {
    return BoxRetentionPolicyAssignment.createAssignmentToMetadata(
        this.getAPI(), this.getID(), templateID, startDateField, fieldFilters);
  }

  /**
   * Updates the information about this retention policy with any info fields that have been
   * modified locally.
   *
  ...
  public BoxRetentionPolicy.Info getInfo(String... fields) {
    ...
    URL url =
        POLICY_URL_TEMPLATE.buildWithQuery(
  ...
  public class Info extends BoxResource.Info {
    ...
    /**
     * Gets the type of the retention policy. A retention policy type can either be "finite", where
     * a specific amount of time to retain the content is known upfront, or "indefinite", where the
    ...
    /**
     * Gets the length of the retention policy. This length specifies the duration in days that the
     * retention policy will be active for after being assigned to content.
     *
    ...
    /**
     * Gets the disposition action of the retention policy. This action can be "permanently_delete",
     * or "remove_retention".
     *
    ...
    /**
     * Gets the status of the retention policy. The status can be "active" or "retired".
     *
    ...
    /**
     * Gets the flag to denote that the owner of a retained file can extend the retention when near
     * expiration.
     *
    ...
    /**
     * Gets the flag to denote that owners and co-owners of a retained file will get notified when
     * near expiration.
     *
     * @return the boolean flag.
     */
     ...
     *     `modifiable` means that you can modify the retention policy. For example, you can add or
     *     remove folders, shorten or lengthen the policy duration, or delete the assignment.
     *     `non-modifiable` means that can modify the retention policy only in a limited way: add a
     *     folder, lengthen the duration, retire the policy, change the disposition action or
     *     notification settings. You cannot perform other actions, such as deleting the assignment
     *     or shortening the policy duration.
     */
    ...
    void parseJSONMember(JsonObject.Member member) {
      ...
      try {
        switch (memberName) {
          ...
          case "retention_type":
            this.retentionType =
                RetentionPolicyParams.RetentionType.fromJSONString(value.asString());
            break;
  ...
  private enum BoxRetentionPolicyType {
    ...
    Finite("finite"),
    /**
     * Type for indefinite retention policies. Indefinite retention policies can have only {@link
     * BoxRetentionPolicyAction#RemoveRetention} assigned action.
     */
