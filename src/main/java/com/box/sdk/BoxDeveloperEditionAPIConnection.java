 ...
 *
 * <p>This class handles everything for Box Developer Edition that isn't already handled by
 * BoxAPIConnection.
 */
public class BoxDeveloperEditionAPIConnection extends BoxAPIConnection {
   ...
   * @param clientSecret the client secret to use when exchanging the JWT assertion for an access
   *     token.
   * @param encryptionPref the encryption preferences for signing the JWT.
   * @param accessTokenCache the cache for storing access token information (to minimize fetching
   *     new tokens)
   */
  ...
  /**
   * Constructs a new BoxDeveloperEditionAPIConnection. Uses {@link InMemoryLRUAccessTokenCache}
   * with a size of 100 to prevent unneeded requests to Box for access tokens.
   *
   * @param entityId enterprise ID or a user ID.
   * @param entityType the type of entityId.
   * @param clientID the client ID to use when exchanging the JWT assertion for an access token.
  ...
  /**
   * Creates a new Box Developer Edition connection with enterprise token leveraging an access token
   * cache.
   *
  ...
  /**
   * Creates a new Box Developer Edition connection with enterprise token. Uses {@link
   * InMemoryLRUAccessTokenCache} with a size of 100 to prevent unneeded requests to Box for access
   * tokens.
   *
   * @param enterpriseId the enterprise ID to use for requesting access token.
   * @param clientId the client ID to use when exchanging the JWT assertion for an access token.
  ...
  /**
   * Creates a new Box Developer Edition connection with enterprise token leveraging BoxConfig and
   * access token cache.
   *
   * @param boxConfig box configuration settings object
   * @param accessTokenCache the cache for storing access token information (to minimize fetching
  ...
  /**
   * Creates a new Box Developer Edition connection with enterprise token leveraging BoxConfig. Uses
   * {@link InMemoryLRUAccessTokenCache} with a size of 100 to prevent unneeded requests to Box for
   * access tokens.
   *
   * @param boxConfig box configuration settings object
   * @return a new instance of BoxAPIConnection.
  ...
  /**
   * Creates a new Box Developer Edition connection with App User or Managed User token leveraging
   * BoxConfig and access token cache.
   *
   * @param userId the user ID to use for an App User.
   * @param boxConfig box configuration settings object
   * @param accessTokenCache the cache for storing access token information (to minimize fetching
   ...
   */
  public static BoxDeveloperEditionAPIConnection getUserConnection(
      String userId, BoxConfig boxConfig, IAccessTokenCache accessTokenCache) {
  ...
  /**
   * Creates a new Box Developer Edition connection with App User or Managed User token. Uses {@link
   * InMemoryLRUAccessTokenCache} with a size of 100 to prevent unneeded requests to Box for access
   * tokens.
   *
   * @param userId the user ID to use for an App User.
   ...
   */
  public static BoxDeveloperEditionAPIConnection getUserConnection(
      String userId, BoxConfig boxConfig) {
    return getUserConnection(
        userId,
        boxConfig.getClientId(),
        boxConfig.getClientSecret(),
  ...
  public void authenticate(String authCode) {
    throw new BoxAPIException(
        "BoxDeveloperEditionAPIConnection does not allow authenticating with an auth code.");
  }
  ...
  public void authenticate() {
    ...
    while (this.backoffCounter.getAttemptsRemaining() > 0) {
      ...
      urlParameters =
          String.format(JWT_GRANT_TYPE, this.getClientID(), this.getClientSecret(), jwtAssertion);
      ...
      try (BoxJSONResponse response = (BoxJSONResponse) request.sendWithoutRetry()) {
        ...
        if (!this.backoffCounter.decrement()
            || (!BoxAPIRequest.isRequestRetryable(apiException)
                && !isResponseRetryable(apiException))) {
          throw apiException;
        }
    ...
    if (this.accessTokenCache != null) {
      ...
      JsonObject accessTokenCacheInfo =
          new JsonObject()
  ...
  private NumericDate getDateForJWTConstruction(
      BoxAPIException apiException, long secondsSinceResponseDateReceived) {
  ...
  private String getAccessTokenCacheKey() {
    return String.format(
        "/%s/%s/%s/%s",
        this.getUserAgent(), this.getClientID(), this.entityType.toString(), this.entityID);
  }
  ...
  public void tryRestoreUsingAccessTokenCache() {
    if (this.accessTokenCache == null) {
      ...
      if (cachedTokenInfo == null) {
        // not found; probably first time for this client config so authenticate; info will then be
        // cached
        this.authenticate();
      } else {
  ...
  private String constructJWTAssertion(NumericDate now) {
    ...
    jws.setKey(
        this.privateKeyDecryptor.decryptPrivateKey(this.privateKey, this.privateKeyPassword));
