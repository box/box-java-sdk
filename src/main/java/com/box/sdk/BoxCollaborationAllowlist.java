...
/**
 * Represents a collaboration allowlist between a domain and a Box Enterprise. Collaboration
 * Allowlist enables a Box Enterprise(only available if you have Box Governance) to manage a set of
 * approved domains that can collaborate with an enterprise.
 *
...
public class BoxCollaborationAllowlist extends BoxResource {
   ...
   * @param direction an enum representing the direction of the collaboration allowlist. Can be set
   *     to inbound, outbound, or both.
   * @return information about the collaboration allowlist created.
   */
  public static BoxCollaborationAllowlist.Info create(
      final BoxAPIConnection api, String domain, AllowlistDirection direction) {
    ...
    JsonObject requestJSON =
        new JsonObject().add("domain", domain).add("direction", direction.toString());
   ...
   */
  public static Iterable<BoxCollaborationAllowlist.Info> getAll(
  ...
  public static Iterable<BoxCollaborationAllowlist.Info> getAll(
    ...
    URL url =
        COLLABORATION_ALLOWLIST_ENTRIES_URL_TEMPLATE.buildWithQuery(
            api.getBaseURL(), builder.toString());
    return new BoxResourceIterable<BoxCollaborationAllowlist.Info>(api, url, limit) {
      ...
      protected BoxCollaborationAllowlist.Info factory(JsonObject jsonObject) {
        BoxCollaborationAllowlist allowlist =
            new BoxCollaborationAllowlist(api, jsonObject.get("id").asString());
  ...
  public BoxCollaborationAllowlist.Info getInfo() {
    URL url =
        COLLABORATION_ALLOWLIST_ENTRY_URL_TEMPLATE.build(this.getAPI().getBaseURL(), this.getID());
  ...
  public enum AllowlistDirection {
     ...
     *
     * @return a String containing information about the direction of the collaboration
     *     allowlisting.
     */
  ...
  public class Info extends BoxResource.Info {
    ...
    /**
     * Get the direction of the collaboration allowlist. Values can be inbound, outbound, or both.
     *
     * @return the direction set for the collaboration allowlist. Values can be inbound, outbound,
     *     or both.
     */
