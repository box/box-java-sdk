...
/**
 * Represents an individual WebLink file on Box. This class can be used to retrieve the link's URL
 * or perform other common file operations (move, copy, delete, etc.).
 *
...
public class BoxWebLink extends BoxItem {
  ...
  /**
   * An array of all possible weblink fields that can be requested when calling {@link
   * #getInfo(String...)}.
   */
  ...
  public BoxWebLink.Info getInfo(String... fields) {
    ...
    if (fields.length > 0) {
      ...
      url =
          WEB_LINK_URL_TEMPLATE.buildWithQuery(
  ...
  /**
   * Updates the information about this weblink with any info fields that have been modified
   * locally.
   *
  ...
  public BoxWebLink.Info setCollections(BoxCollection... collections) {
    ...
    URL url =
        WEB_LINK_URL_TEMPLATE.buildWithQuery(this.getAPI().getBaseURL(), queryString, this.getID());
    BoxJSONRequest request = new BoxJSONRequest(this.getAPI(), url, "PUT");
