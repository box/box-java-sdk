...
public class BoxGroup extends BoxCollaborator {
  ...
  /**
   * Gets an iterable of all the groups in the enterprise that are starting with the given name
   * string.
   *
   * @param api the API connection to be used when retrieving the groups.
   * @param name the name prefix of the groups. If the groups need to searched by full name that has
   *     spaces, then the parameter string should have been wrapped with "".
   * @param fields the fields to retrieve.
   * @return an iterable containing info about all the groups.
   */
  ...
  public Info getInfo(String... fields) {
    ...
    if (fields.length > 0) {
      ...
      url =
          GROUP_URL_TEMPLATE.buildWithQuery(
  ...
  public Collection<BoxGroupMembership.Info> getMemberships() {
    ...
    // We need to iterate all results because this method must return a Collection. This logic
  ...
  /**
   * Gets information about all of the group memberships for this group as iterable with paging
   * support.
   *
   * @param fields the fields to retrieve.
  ...
  public Iterable<BoxGroupMembership.Info> getAllMemberships(String... fields) {
    ...
    return () -> {
      URL url =
          MEMBERSHIPS_URL_TEMPLATE.buildWithQuery(
   ...
   * @param user the member to be added to this group.
   * @param role the role of the user in this group. Can be null to assign the default role.
   * @param configurablePermissions the configurable permission of the user as a group admin. Can be
   *     null to give all group admin permissions.
   * @return info about the new group membership.
   */
  public BoxGroupMembership.Info addMembership(
      BoxUser user,
    ...
    try (BoxJSONResponse response = request.send()) {
      ...
      BoxGroupMembership membership =
          new BoxGroupMembership(api, responseJSON.get("id").asString());
  ...
  public Iterable<BoxCollaboration.Info> getAllCollaborations(String... fields) {
    ...
    return () -> {
      URL url =
          COLLABORATIONS_URL_TEMPLATE.buildWithQuery(
              api.getBaseURL(), builder.toString(), BoxGroup.this.getID());
