...
public abstract class BoxResourceIterable<T> implements Iterable<T> {
   ...
   * @param marker the marker where the iterator will begin
   */
  public BoxResourceIterable(
      BoxAPIConnection api, URL url, int limit, JsonObject body, String marker) {
    this.api = api;
    this.url = url;
    this.limit = limit;
  ...
  private class IteratorImpl implements Iterator<T> {
    ...
    /**
     * Base 64 encoded string that represents where the paging should being. It should be left blank
     * to begin paging.
     */
