...
public class BoxWebHook extends BoxResource {
  ...
  public static BoxWebHook.Info create(
      BoxResource target, URL address, BoxWebHook.Trigger... triggers) {
   ...
   */
  public static BoxWebHook.Info create(
      BoxResource target, URL address, Set<BoxWebHook.Trigger> triggers) {
    ...
    JsonObject targetJSON =
        new JsonObject().add(JSON_KEY_TARGET_TYPE, type).add(JSON_KEY_TARGET_ID, target.getID());
  ...
  /**
   * Validates that provided {@link BoxWebHook.Trigger}-s can be applied on the provided {@link
   * BoxResourceType}.
   *
  ...
  /**
   * Validates that provided {@link BoxWebHook.Trigger} can be applied on the provided {@link
   * BoxResourceType}.
   *
  ...
  private static void validateTrigger(String targetType, BoxWebHook.Trigger trigger) {
    ...
    throw new IllegalArgumentException(
        String.format(
            "Provided trigger '%s' is not supported on provided target '%s'.",
            trigger.name(), targetType));
  }
  ...
  public BoxWebHook.Info getInfo(String... fields) {
    ...
    if (fields.length > 0) {
      ...
      url =
          WEBHOOK_URL_TEMPLATE.buildWithQuery(
  ...
  public enum Trigger {
    ...
    FILE_COPIED(
        ...
        BoxResource.getResourceType(BoxFile.class)),
    ...
    void parseJSONMember(JsonObject.Member member) {
      ...
      try {
        if (memberName.equals(JSON_KEY_TARGET)) {
          ...
          this.triggers =
              new HashSet<>(CollectionUtils.map(value.asArray().values(), JSON_VALUE_TO_TRIGGER));
