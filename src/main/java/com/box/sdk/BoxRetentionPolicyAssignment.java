...
public class BoxRetentionPolicyAssignment extends BoxResource {
  ...
  public static final URLTemplate ASSIGNMENTS_URL_TEMPLATE =
      new URLTemplate("retention_policy_assignments");
  ...
  /**
   * The URL template used for operation with files under retention with given retention policy
   * assignment ID.
   */
  ...
  /**
   * The URL template used for operation with file versions under retention with given retention
   ...
   */
  public static BoxRetentionPolicyAssignment.Info createAssignmentToEnterprise(
      BoxAPIConnection api, String policyID) {
    return createAssignment(
        api, policyID, new JsonObject().add("type", TYPE_ENTERPRISE), null, null);
  }
   ...
   * @return info about created assignment.
   */
  public static BoxRetentionPolicyAssignment.Info createAssignmentToFolder(
      BoxAPIConnection api, String policyID, String folderID) {
    return createAssignment(
        api, policyID, new JsonObject().add("type", TYPE_FOLDER).add("id", folderID), null, null);
  }

  /**
   * Assigns a retention policy to all items with a given metadata template, optionally matching on
   * fields.
   *
   * @param api the API connection to be used by the created assignment.
   * @param policyID id of the assigned retention policy.
   ...
   */
  public static BoxRetentionPolicyAssignment.Info createAssignmentToMetadata(
      BoxAPIConnection api, String policyID, String templateID, MetadataFieldFilter... filter) {
   ...
   * @return info about created assignment.
   */
  private static BoxRetentionPolicyAssignment.Info createAssignment(
      BoxAPIConnection api,
      String policyID,
      JsonObject assignTo,
      String startDateField,
  ...
  public BoxRetentionPolicyAssignment.Info getInfo(String... fields) {
    ...
    URL url =
        RETENTION_POLICY_ASSIGNMENT_URL_TEMPLATE.buildWithQuery(
  ...
  public Iterable<BoxFile.Info> getFilesUnderRetention(int limit, String... fields) {
    ...
    URL url =
        FILES_UNDER_RETENTION_URL_TEMPLATE.buildWithQuery(
  ...
  public Iterable<BoxFile.Info> getFileVersionsUnderRetention(int limit, String... fields) {
    ...
    URL url =
        FILE_VERSIONS_UNDER_RETENTION_URL_TEMPLATE.buildWithQuery(
            getAPI().getBaseURL(), queryString.toString(), getID());
    return new BoxResourceIterable<BoxFile.Info>(getAPI(), url, limit) {
      @Override
      protected BoxFile.Info factory(JsonObject jsonObject) {
  ...
  public void delete() {
    URL url =
        RETENTION_POLICY_ASSIGNMENT_URL_TEMPLATE.build(this.getAPI().getBaseURL(), this.getID());
