...
/**
 * Collections contain information about the items contained inside of them, including files and
...
public class BoxCollection extends BoxResource implements Iterable<BoxItem.Info> {
  ...
  public static final URLTemplate GET_COLLECTION_ITEMS_URL =
      new URLTemplate("collections/%s/items/");
  ...
  /**
   * Returns an iterable containing the items in this collection. Iterating over the iterable
   * returned by this method is equivalent to iterating over this BoxCollection directly.
   *
  ...
  /**
   * Returns an iterable containing the items in this collection and specifies which attributes to
  ...
  public Iterable<BoxItem.Info> getItems(final String... fields) {
    return () -> {
      ...
      URL url =
          GET_COLLECTION_ITEMS_URL.buildWithQuery(getAPI().getBaseURL(), queryString, getID());
  ...
  public PartialCollection<BoxItem.Info> getItemsRange(long offset, long limit, String... fields) {
    QueryStringBuilder builder =
        new QueryStringBuilder().appendParam("offset", offset).appendParam("limit", limit);
    ...
    URL url =
        GET_COLLECTION_ITEMS_URL.buildWithQuery(getAPI().getBaseURL(), builder.toString(), getID());
    ...
    try (BoxJSONResponse response = request.send()) {
      ...
      for (JsonValue entry : entries) {
        BoxItem.Info entryInfo =
            (BoxItem.Info) BoxResource.parseInfo(this.getAPI(), entry.asObject());
        if (entryInfo != null) {
          items.add(entryInfo);
        }
      }
  ...
  public Iterator<BoxItem.Info> iterator() {
    URL url =
        GET_COLLECTION_ITEMS_URL.build(this.getAPI().getBaseURL(), BoxCollection.this.getID());
