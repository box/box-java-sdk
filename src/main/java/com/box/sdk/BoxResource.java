...
/**
 * The abstract base class for all resource types (files, folders, comments, collaborations, etc.)
 * used by the API.
 *
 * <p>Every API resource has an ID and a {@link BoxAPIConnection} that it uses to communicate with
 * the API. Some resources also have an associated {@link Info} class that contains information
 * about the resource.
 */
public abstract class BoxResource {

  /** @see #initResourceClassByType() */
  private static final Map<String, Class<? extends BoxResource>> RESOURCE_CLASS_BY_TYPE =
      initResourceClassByType();
  ...
  /**
   * @return Builds {@link Map} between String {@link #getResourceType(Class)} and {@link
   *     BoxResource} type.
   */
  private static Map<String, Class<? extends BoxResource>> initResourceClassByType() {
    ...
    result.put(
        getResourceType(BoxRetentionPolicyAssignment.class), BoxRetentionPolicyAssignment.class);
    ...
    result.put(getResourceType(BoxFileRequest.class), BoxFileRequest.class);
  ...
  public static String getResourceType(Class<? extends BoxResource> clazz) {
    ...
    if (resource == null) {
      throw new IllegalArgumentException(
          "Provided BoxResource type does not have @BoxResourceType annotation.");
    }
  ...
  static BoxResource.Info parseInfo(BoxAPIConnection api, JsonObject jsonObject) {
    ...
    try {
      ...
      Class<?> infoClass =
          resourceClass.getClassLoader().loadClass(resourceClass.getCanonicalName() + "$Info");
      Constructor<?> infoConstructor =
          infoClass.getDeclaredConstructor(resourceClass, JsonObject.class);
  ...
  /**
   * Indicates whether this BoxResource is equal to another BoxResource. Two BoxResources are equal
