...
/**
 * Class describing request to get Admin Logs Streaming. You can use it's fluent interface to create
 * new request like so:
 *
 * <pre>{@code
...
public final class EnterpriseEventsStreamRequest {
  ...
  public EnterpriseEventsStreamRequest types(EventType... types) {
    return typeNames(Arrays.stream(types).map(EventType::toJSONString).toArray(String[]::new));
  }
