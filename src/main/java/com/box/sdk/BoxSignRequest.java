...
/**
 * Represents a Sign Request used by Box Sign. Sign Requests are used to request e-signatures on
 * documents from signers. A Sign Request can refer to one or more Box Files and can be sent to one
 * or more Box Sign Request Signers.
 *
...
public class BoxSignRequest extends BoxResource {
   ...
   */
  public static BoxSignRequest.Info createSignRequestFromFiles(
    ...
    return createSignRequest(
        api, toBoxSignRequestFiles(sourceFiles), signers, parentFolderId, optionalParams);
  }
  ...
  public static BoxSignRequest.Info createSignRequestFromFiles(
    ...
    return createSignRequest(
        api, toBoxSignRequestFiles(sourceFiles), signers, parentFolderId, null);
  }
   ...
   * @return the created sign request's info.
   */
  public static BoxSignRequest.Info createSignRequest(
      BoxAPIConnection api,
   ...
   * @return an iterable with all the sign requests.
   */
  public static Iterable<BoxSignRequest.Info> getAll(
      final BoxAPIConnection api, int limit, String... fields) {
  ...
  public BoxSignRequest.Info getInfo(String... fields) {
    ...
    URL url =
        SIGN_REQUEST_URL_TEMPLATE.buildAlphaWithQuery(
  ...
  /**
   * Cancels a sign request if it has not yet been signed or declined. Any outstanding signers will
   * no longer be able to sign the document.
   *
   * @return the cancelled sign request's info.
   */
  public BoxSignRequest.Info cancel() {
    URL url =
        SIGN_REQUEST_CANCEL_URL_TEMPLATE.buildAlphaWithQuery(
            getAPI().getBaseURL(), "", this.getID());
  ...
  /**
   * Attempts to resend a Sign Request to all signers that have not signed yet. There is a 10 minute
   * cooling-off period between each resend request. If you make a resend call during the
   * cooling-off period, a BoxAPIException will be thrown.
   */
  public void resend() {
    URL url =
        SIGN_REQUEST_RESEND_URL_TEMPLATE.buildAlphaWithQuery(
            getAPI().getBaseURL(), "", this.getID());
  ...
  public enum BoxSignRequestStatus {
    ...
    static BoxSignRequestStatus fromJSONString(String jsonValue) {
      ...
      throw new IllegalArgumentException(
          "The provided JSON value isn't a valid BoxSignRequestStatus value.");
    }
  }
  ...
  public class Info extends BoxResource.Info {
    ...
    /**
     * Gets the flag indicating if sending reminders for signers to sign a document on day 3, 8, 13
     * and 18 (or less if the document has been digitally signed already) is enabled.
     *
    ...
    /**
     * Gets the destination folder to place sign request specific data in (copy of source files,
     * signing log etc.).
     *
    ...
    /**
     * Gets the URL that can be used by the sign request sender to prepare the document through the
     * UI.
     *
     * @return prepare url.
     */
    ...
    /**
     * List of files that will be signed, which are copies of the original source files. A new
     * version of these files are created as signers sign and can be downloaded at any point in the
     * signing process.
     *
     * @return sign files.
     */
    ...
    /**
     * Uses days_valid to calculate the date and time that the sign request will expire, if
     * unsigned.
     *
        ...
        switch (memberName) {
          ...
          case "signers":
            ...
            for (JsonValue signerJSON : value.asArray()) {
              BoxSignRequestSigner signer =
                  new BoxSignRequestSigner(signerJSON.asObject(), getAPI());
              signers.add(signer);
            }
    ...
    public class BoxSignRequestSignFiles {
      ...
      /**
       * Gets the list of files that signing events will occur on - these are copies of the original
       * source files.
       *
      ...
      /**
       * Gets the flag indicating whether a change to the document is processing and the PDF may be
       * out of date. It is recommended to wait until processing has finished before downloading the
       * PDF. Webhooks are not sent until processing has been completed.
       *
