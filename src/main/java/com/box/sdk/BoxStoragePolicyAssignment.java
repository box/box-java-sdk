...
public class BoxStoragePolicyAssignment extends BoxResource {
  ...
  public static final URLTemplate STORAGE_POLICY_ASSIGNMENT_URL_TEMPLATE =
      new URLTemplate("storage_policy_assignments");

  /** Storage Policy Assignment URL Template. */
  public static final URLTemplate STORAGE_POLICY_ASSIGNMENT_WITH_ID_URL_TEMPLATE =
      new URLTemplate("storage_policy_assignments/%s");
   ...
   */
  public static BoxStoragePolicyAssignment.Info create(
      BoxAPIConnection api, String policyID, String userID) {
    ...
    JsonObject requestJSON =
        new JsonObject()
            .add(
                "storage_policy",
                new JsonObject().add("type", "storage_policy").add("id", policyID))
            .add("assigned_to", new JsonObject().add("type", "user").add("id", userID));
    ...
    try (BoxJSONResponse response = request.send()) {
      ...
      BoxStoragePolicyAssignment storagePolicyAssignment =
          new BoxStoragePolicyAssignment(api, responseJSON.get("id").asString());
   ...
   */
  public static BoxStoragePolicyAssignment.Info getAssignmentForTarget(
      final BoxAPIConnection api, String resolvedForType, String resolvedForID) {
    QueryStringBuilder builder = new QueryStringBuilder();
    builder
        .appendParam("resolved_for_type", resolvedForType)
        .appendParam("resolved_for_id", resolvedForID);
    URL url =
        STORAGE_POLICY_ASSIGNMENT_URL_TEMPLATE.buildWithQuery(api.getBaseURL(), builder.toString());
    BoxJSONRequest request = new BoxJSONRequest(api, url, HttpMethod.GET);
    try (BoxJSONResponse response = request.send()) {

      BoxStoragePolicyAssignment storagePolicyAssignment =
          new BoxStoragePolicyAssignment(
              api,
              response
                  .getJsonObject()
                  .get("entries")
   ...
   * @return information about this {@link BoxStoragePolicyAssignment}.
   */
  public static BoxStoragePolicyAssignment.Info assign(
      BoxAPIConnection api, String storagePolicyID, String userID) {
    ...
    BoxStoragePolicyAssignment assignment =
        new BoxStoragePolicyAssignment(api, assignmentInfo.getID());
    assignment.updateInfo(assignmentInfo);
    return assignmentInfo;
  }

  /**
   * Updates the information about the BoxStoragePolicyAssignment with any info fields that have
  ...
  public void updateInfo(BoxStoragePolicyAssignment.Info info) {
    URL url =
        STORAGE_POLICY_ASSIGNMENT_WITH_ID_URL_TEMPLATE.buildAlpha(
            this.getAPI().getBaseURL(), this.getID());
