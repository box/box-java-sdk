 ...
 *
 * <p>All requests to the REST API are sent using this class or one of its subclasses. This class
 * wraps {@link HttpURLConnection} in order to provide a simpler interface that can automatically
 * handle various conditions specific to Box's API. Requests will be authenticated using a {@link
 * BoxAPIConnection} (if one is provided), so it isn't necessary to add authorization headers.
 * Requests can also be sent more than once, unlike with HttpURLConnection. If an error occurs while
 * sending a request, it will be automatically retried (with a back off delay) up to the maximum
 * number of times set in the BoxAPIConnection.
 *
 * <p>Specifying a body for a BoxAPIRequest is done differently than it is with HttpURLConnection.
 * Instead of writing to an OutputStream, the request is provided an {@link InputStream} which will
 * be read when the {@link #send} method is called. This makes it easy to retry requests since the
 * stream can automatically reset and reread with each attempt. If the stream cannot be reset, then
 * a new stream will need to be provided before each call to send. There is also a convenience
 * method for specifying the body as a String, which simply wraps the String with an InputStream.
 */
public class BoxAPIRequest {
   ...
   *
   * <p>Note that any bytes that read from the returned stream won't be sent unless the stream is
   * reset back to its initial position.
   *
   ...
   *
   * <p>The stream must support the {@link InputStream#reset} method if auto-retry is used or if the
   * request needs to be resent. Otherwise, the body must be manually set before each call to {@link
   * #send}.
   *
   ...
   *
   * <p>If the contents of the body are large, then it may be more efficient to use an {@link
   * InputStream} instead of a String. Using a String requires that the entire body be in memory
   ...
   * Sets the request body to the contents of an InputStream.
   *
   * <p>Providing the length of the InputStream allows for the progress of the request to be
   * monitored when calling {@link #send(ProgressListener)}.
   *
   * <p>See {@link #setBody(InputStream)} for more information on setting the body of the request.
   *
   * @param stream an InputStream containing the contents of the body.
   ...
   *
   * <p>The type of the returned BoxAPIResponse will be based on the content type returned by the
   * server, allowing it to be cast to a more specific type. For example, if it's known that the API
   * call will return a JSON response, then it can be cast to a {@link BoxJSONResponse} like so:
   *
   ...
   *
   * <p>If the server returns an error code or if a network error occurs, then the request will be
   * automatically retried. If the maximum number of retries is reached and an error still occurs,
   * then a {@link BoxAPIException} will be thrown.
   *
   * <p>See {@link #send} for more information on sending requests.
   *
   * @return a {@link BoxAPIResponse} containing the server's response.
   * @throws BoxAPIException if the server returns an error code or if a network error occurs.
   */
  ...
  /**
   * Sends this request while monitoring its progress and returns a BoxAPIResponse containing the
   * server's response.
   *
   * <p>The type of the returned BoxAPIResponse will be based on the content type returned by the
   * server, allowing it to be cast to a more specific type. For example, if it's known that the API
   ...
   * then a {@link BoxAPIException} will be thrown.
   *
   * <p>A ProgressListener is generally only useful when the size of the request is known
   * beforehand. If the size is unknown, then the ProgressListener will be updated for each byte
   * sent, but the total number of bytes will be reported as 0.
   *
   * <p>See {@link #send} for more information on sending requests.
   *
  ...
  public BoxAPIResponse send(ProgressListener listener) {
    ...
    while (this.backoffCounter.getAttemptsRemaining() > 0) {
      try {
        ...
        LOGGER.warn(
            format(
                "Retrying request due to transient error status=%d body=%s headers=%s",
  ...
  /**
   * Disables adding authentication header to request. Useful when you want to add your own
   * authentication method. Default value is `true` and SKD will add authenticaton header to
   * request.
   *
  ...
  /**
   * Sends a request to upload a file part and returns a BoxFileUploadSessionPart containing
   * information about the upload part. This method is separate from send() because it has custom
   * retry logic.
   *
   * <p>If the server returns an error code or if a network error occurs, then the request will be
   * automatically retried. If the maximum number of retries is reached and an error still occurs,
      ...
      try (BoxJSONResponse response = (BoxJSONResponse) this.trySend(null)) {
        ...
        LOGGER.warn(
            format(
                ...
                apiException.getResponseCode(), apiException.getResponse()));
  ...
  private String toStringWithHeaders(Headers headers) {
    ...
    if (headers != null) {
      ...
      sanitizeHeaders(headers)
          .forEach(
              h ->
                  builder.append(format("%s: [%s]%s", h.getFirst(), h.getSecond(), lineSeparator)));
    }
  ...
  /**
   * Returns a String representation of this request's body used in {@link #toString}. This method
   * returns null by default.
   *
   * <p>A subclass may want override this method if the body can be converted to a String for
   ...
   *
   * <p>This method will be called before each attempt to resend the request, giving subclasses an
   * opportunity to reset any streams that need to be read when sending the body.
   *
  ...
  private BoxAPIResponse trySend(ProgressListener listener) {
    ...
    logDebug(
        format("[trySend] connection.connect() took %dms%n", (System.currentTimeMillis() - start)));

    BoxAPIResponse result = BoxAPIResponse.toBoxResponse(response);
    long getResponseStart = System.currentTimeMillis();
    logDebug(
        format(
            "[trySend] Get Response (read network) took %dms%n",
            System.currentTimeMillis() - getResponseStart));
    return result;
  }
  ...
  private Request composeRequest(ProgressListener listener) {
    ...
    try {
      ...
      if (this.api instanceof SharedLinkAPIConnection) {
        ...
        String boxAPIValue =
            BoxSharedLink.getSharedLinkHeaderValue(
                sharedItemAPI.getSharedLink(), sharedItemAPI.getSharedLinkPassword());
  ...
  protected void writeMethodWithBody(Request.Builder requestBuilder, ProgressListener listener) {
    ...
    if (body != null) {
      ...
      logDebug(
          format("[trySend] Body write took %dms%n", (System.currentTimeMillis() - writeStart)));
    }
