...
/**
 * Represents a legal hold policy assignment. Legal hold assignments are used to assign legal hold
 * policies to custodians, folders, files, or file versions.
 *
 * @see <a href="https://developer.box.com/reference/resources/legal-hold-policy-assignment/">Box
 *     legal holds</a>
...
public class BoxLegalHoldAssignment extends BoxResource {
  ...
  public static final String TYPE_FILE_VERSION =
      BoxFileVersion.getResourceType(BoxFileVersion.class);
  ...
  public static final URLTemplate ASSIGNMENTS_URL_TEMPLATE =
      new URLTemplate("legal_hold_policy_assignments");
   ...
   * @param resourceType type of target resource. Can be 'file_version', 'file', 'folder', or
   *     'user'.
   * @param resourceID ID of the target resource.
   * @return info about created legal hold policy assignment.
   */
  public static BoxLegalHoldAssignment.Info create(
      BoxAPIConnection api, String policyID, String resourceType, String resourceID) {
    ...
    JsonObject requestJSON =
        new JsonObject()
            ...
            .add("assign_to", new JsonObject().add("type", resourceType).add("id", resourceID));
    request.setBody(requestJSON.toString());
    try (BoxJSONResponse response = request.send()) {
      JsonObject responseJSON = Json.parse(response.getJSON()).asObject();
      BoxLegalHoldAssignment createdAssignment =
          new BoxLegalHoldAssignment(api, responseJSON.get("id").asString());
  ...
  public BoxLegalHoldAssignment.Info getInfo(String... fields) {
    ...
    URL url =
        LEGAL_HOLD_ASSIGNMENT_URL_TEMPLATE.buildWithQuery(
