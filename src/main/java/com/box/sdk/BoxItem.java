...
public abstract class BoxItem extends BoxResource {
  ...
  /**
   * Used to retrieve the watermark for the item. If the item does not have a watermark applied to
  ...
  protected BoxWatermark applyWatermark(URLTemplate itemUrl, String imprint) {
    ...
    JsonObject body =
        new JsonObject()
            .add(
                BoxWatermark.WATERMARK_JSON_KEY,
                new JsonObject().add(BoxWatermark.WATERMARK_IMPRINT_JSON_KEY, imprint));
  ...
  /**
   * Copies this item to another folder and gives it a new name. If the destination is the same
   * folder as the item's current parent, then newName must be a new, unique name.
   *
   * @param destination the destination folder.
  ...
  public abstract class Info extends BoxResource.Info {
     ...
     *
     * <p>Note that this field isn't populated by default and must be specified as a field parameter
     * when getting Info about the item.
     *
