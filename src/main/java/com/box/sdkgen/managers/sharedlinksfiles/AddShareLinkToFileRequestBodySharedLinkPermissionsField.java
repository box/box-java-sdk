...
public class AddShareLinkToFileRequestBodySharedLinkPermissionsField extends SerializableObject {
  ...
  /**
   * If the shared link allows for editing of files. This can only be set when `access` is set to
   * `open` or `company`. This value can only be `true` is `can_download` is also `true`.
   */
