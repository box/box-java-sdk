...
public class SearchResultWithSharedLinkItem extends OneOfThree<FileFull, FolderFull, WebLink> {
  ...
  static class SearchResultWithSharedLinkItemDeserializer
      extends JsonDeserializer<SearchResultWithSharedLinkItem> {

    public SearchResultWithSharedLinkItemDeserializer() {
      super();
    }

    @Override
    public SearchResultWithSharedLinkItem deserialize(JsonParser jp, DeserializationContext ctxt)
        throws IOException {
      ...
      if (!(discriminant0 == null)) {
        switch (discriminant0.asText()) {
          case "file":
            return new SearchResultWithSharedLinkItem(
                JsonManager.deserialize(node, FileFull.class));
          case "folder":
            return new SearchResultWithSharedLinkItem(
                JsonManager.deserialize(node, FolderFull.class));
          case "web_link":
            return new SearchResultWithSharedLinkItem(JsonManager.deserialize(node, WebLink.class));
        }
      }
      throw new JsonMappingException(jp, "Unable to deserialize SearchResultWithSharedLinkItem");
    }
  }
}
