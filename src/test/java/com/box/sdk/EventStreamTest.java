...
public class EventStreamTest {
  ...
  public void canStopStreamWhileWaitingForAPIResponse() throws InterruptedException {
    ...
    stubFor(
        options(urlEqualTo(eventsURL))
            .willReturn(
                aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
                    .withBody(
                        "{ \"entries\": [ { \"url\": \"https://localhost:"
                            + wireMockRule.httpsPort()
                            + realtimeServerURL
                            + "\", \"max_retries\": \"3\", \"retry_timeout\": 60000 } ] }")));

    stubFor(
        get(urlPathMatching(eventsURL))
            .withQueryParam("stream_position", WireMock.equalTo("now"))
            .willReturn(
                aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
                    .withBody(
                        "{ \"next_stream_position\": " + streamPosition + ",\"entries\":[] }")));

    stubFor(
        get(urlMatching("/2.0/realtimeServer.*"))
            .willReturn(
                aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
    ...
    this.wireMockRule.addMockServiceRequestListener(
        (request, response) -> {
  ...
  public void duplicateEventsAreNotReported() throws InterruptedException {
    ...
    stubFor(
        get(urlMatching("/2.0/events\\?.*stream_position=now.*"))
            .willReturn(
                aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
    ...
    stubFor(
        get(urlMatching("/2.0/events\\?.*stream_position=0"))
            .willReturn(
                aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
                    .withBody(
                        "{ \"next_stream_position\": 1, \"entries\": [ { \"type\": \"event\", "
                            + "\"event_id\": \"1\" } ] }")));

    stubFor(
        get(urlMatching("/2.0/events\\?.*stream_position=1"))
            .willReturn(
                aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
                    .withBody(
  ...
  public void delayBetweenCalls() throws InterruptedException {
    ...
    stubFor(
        get(urlMatching("/2.0/events\\?.*stream_position=123.*"))
            .willReturn(
                aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
                    .withBody(
                        "{ \"next_stream_position\": 456, \"entries\": [ { \"type\": \"event\", "
                            + "\"event_id\": \"1\" } ] }")));

    stubFor(
        get(urlMatching("/2.0/events\\?.*stream_position=456.*"))
            .willReturn(
                aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
                    .withBody(
                        "{ \"next_stream_position\": 789, \"entries\": [ { \"type\": \"event\", "
                            + "\"event_id\": \"1\" } ] }")));
