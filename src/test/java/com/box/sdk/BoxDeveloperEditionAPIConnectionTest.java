...
public class BoxDeveloperEditionAPIConnectionTest {
  ...
  public void retriesWithNewJWTAssertionOnErrorResponseAndFails() {
    ...
    this.wireMockRule.stubFor(
        requestMatching(this.getRequestMatcher(tokenPath))
            .atPriority(2)
  ...
  @Test
  public void usesCustomDecryptorClassImplementation()
  ...
  private BoxDeveloperEditionAPIConnection getBoxDeveloperEditionAPIConnection(
      IPrivateKeyDecryptor decryptor) {
    ...
    String privateKey =
        "LS0tLS1CRUdJTiBSU0EgUFJJVkFURSBLRVktLS0tLQpQcm9jLVR5cGU6IDQsRU5DUllQVEVECkRFSy1JbmZvOiBB"
    ...
    BoxDeveloperEditionAPIConnection api =
        new BoxDeveloperEditionAPIConnection(
            "12345", DeveloperEditionEntityType.USER, "foo", "bar", prefs, null);
    ...
    return new RequestMatcherExtension() {
      ...
      public MatchResult match(Request request, Parameters parameters) {
        if (!request.getMethod().equals(RequestMethod.POST)
            || !request.getUrl().equals(tokenPath)) {
          return MatchResult.noMatch();
        }

        Assert.assertNotNull(
            "JTI should be saved from previous request",
            BoxDeveloperEditionAPIConnectionTest.this.jtiClaim);

        try {
          JwtClaims claims =
              BoxDeveloperEditionAPIConnectionTest.this.getClaimsFromRequest(request);
          ...
          Assert.assertNotEquals(
              "JWT should have a new jti claim",
              BoxDeveloperEditionAPIConnectionTest.this.jtiClaim,
              jti);
