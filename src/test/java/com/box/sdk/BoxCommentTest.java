...
public class BoxCommentTest {
  ...
  public void testDeleteACommentSucceeds() {
    ...
    wireMockRule.stubFor(
        WireMock.delete(WireMock.urlPathEqualTo(deleteCommentURL))
            .willReturn(
  ...
  public void testChangeACommentsMessageSucceedsAndSendCorrectJson() {
    ...
    JsonObject updateCommentObject = new JsonObject().add("message", updatedMessage);

    String result = TestUtils.getFixture("BoxComment/UpdateCommentsMessage200");

    wireMockRule.stubFor(
        WireMock.put(WireMock.urlPathEqualTo(changeCommentURL))
  ...
  public void testCreateCommentSucceedsAndSendsCorrectJson() {
    ...
    JsonObject itemObject = new JsonObject().add("type", "file").add("id", fileID);

    JsonObject postCommentObject =
        new JsonObject().add("item", itemObject).add("message", testCommentMesssage);

    String result = TestUtils.getFixture("BoxComment/CreateComment201");

    wireMockRule.stubFor(
        WireMock.post(WireMock.urlPathEqualTo(createCommentURL))
  ...
  public void testGetCommentsOnFileSucceeds() {
    ...
    wireMockRule.stubFor(
        WireMock.get(WireMock.urlPathEqualTo(fileCommentURL))
            .willReturn(
                WireMock.aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
                    .withBody(result)));
  ...
  public void testGetCommentInfoSucceeds() {
    ...
    wireMockRule.stubFor(
        WireMock.get(WireMock.urlPathEqualTo(getCommentURL))
            .willReturn(
                WireMock.aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
                    .withBody(result)));
