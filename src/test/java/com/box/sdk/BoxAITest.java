...
public class BoxAITest {
  ...
  public void testSendAIRequestSuccess() {
    ...
    wireMockRule.stubFor(
        WireMock.post(WireMock.urlPathEqualTo("/2.0/ai/ask"))
            .willReturn(
                WireMock.aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
                    .withBody(result)));

    BoxAIResponse response =
        BoxAI.sendAIRequest(
            api,
            prompt,
    ...
    assertThat(
        response.getAnswer(),
        equalTo("Public APIs are important because of key and important reasons."));
  ...
  public void testSendAITexGenRequestWithNoDialogueHistorySuccess() {
    ...
    String expectedRequestBody =
        String.format(
            "{\"prompt\": \"%s\", \"items\": [{\"id\": \"%s\", \"type\": \"file\"}]}",
            prompt, fileId);

    String result = TestUtils.getFixture("BoxAI/SendAITextGen200");
    wireMockRule.stubFor(
        WireMock.post(WireMock.urlPathEqualTo("/2.0/ai/text_gen"))
    ...
    BoxAIResponse response =
        BoxAI.sendAITextGenRequest(
            api, prompt, Collections.singletonList(new BoxAIItem(fileId, BoxAIItem.Type.FILE)));
  ...
  public void testSendAITexGenRequestWithDialogueHistorySuccess() throws ParseException {
    ...
    dialogueHistory.add(new BoxAIDialogueEntry("What is the size of the file?", "10kb", date2));
  ...
  public void testGetAIAgentDefaultConfigAskSuccess() {
    ...
    BoxAIAgent agent =
        BoxAI.getAiAgentDefaultConfig(api, BoxAIAgent.Mode.ASK, "en", "openai__gpt_3_5_turbo");
  ...
  public void testGetAIAgentDefaultConfigTextGenSuccess() {
    ...
    BoxAIAgent agent =
        BoxAI.getAiAgentDefaultConfig(api, BoxAIAgent.Mode.TEXT_GEN, "en", "openai__gpt_3_5_turbo");
  ...
  public void testGetAIAgentDefaultConfigExtractSuccess() {
    ...
    BoxAIAgent agent =
        BoxAI.getAiAgentDefaultConfig(api, BoxAIAgent.Mode.EXTRACT, "en", "openai__gpt_3_5_turbo");
  ...
  public void testGetAIAgentDefaultConfigExtractStructuredSuccess() {
    ...
    BoxAIAgent agent =
        BoxAI.getAiAgentDefaultConfig(
            api, BoxAIAgent.Mode.EXTRACT_STRUCTURED, "en", "openai__gpt_3_5_turbo");
  ...
  public void testExtractMetadataFreeformSuccess() {
    ...
    expectedRequestBody.add(
        "items", new JsonArray().add(new JsonObject().add("type", "file").add("id", fileId)));
    expectedRequestBody.add("ai_agent", agent.getJSONObject());

    String result = TestUtils.getFixture("BoxAI/ExtractMetadataFreeform200");
    wireMockRule.stubFor(
        WireMock.post(WireMock.urlPathEqualTo("/2.0/ai/extract"))
            .withRequestBody(WireMock.equalToJson(expectedRequestBody.toString()))
            .willReturn(
                WireMock.aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
                    .withBody(result)));
  ...
  public void testExtractMetadataStructuredWithTemplateSuccess() {
    ...
    final String fileId = "12345";
    final BoxAIExtractMetadataTemplate template =
        new BoxAIExtractMetadataTemplate("templateId", "enterprise");
    final List<BoxAIItem> items =
        Collections.singletonList(new BoxAIItem(fileId, BoxAIItem.Type.FILE));
    final String result = TestUtils.getFixture("BoxAI/ExtractMetadataStructured200");
    final BoxAIAgentExtractStructured agent =
        new BoxAIAgentExtractStructured(Json.parse(agentString).asObject());

    final JsonObject expectedRequestBody =
        new JsonObject()
            .add(
                "items",
                new JsonArray().add(new JsonObject().add("type", "file").add("id", fileId)))
            .add(
                "metadata_template",
                new JsonObject()
    ...
    wireMockRule.stubFor(
        WireMock.post(WireMock.urlPathEqualTo("/2.0/ai/extract_structured"))
            .withRequestBody(WireMock.equalToJson(expectedRequestBody.toString()))
            .willReturn(
                WireMock.aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
    ...
    BoxAIExtractStructuredResponse response =
        BoxAI.extractMetadataStructured(api, items, template, agent);
  ...
  public void testExtractMetadataStructuredWithFieldSuccess() {
    ...
    final BoxAIAgentExtractStructured agent =
        new BoxAIAgentExtractStructured(Json.parse(agentString).asObject());
    final BoxAIExtractField field =
        new BoxAIExtractField(
            "text",
            "The name of the file",
            "Name",
            "name",
            Arrays.asList(
                new BoxAIExtractFieldOption("option 1"), new BoxAIExtractFieldOption("option 2")),
    ...
    BoxAIExtractStructuredResponse response =
        BoxAI.extractMetadataStructured(api, items, Collections.singletonList(field), agent);
