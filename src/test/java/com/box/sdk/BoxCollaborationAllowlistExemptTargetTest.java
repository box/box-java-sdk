...
public class BoxCollaborationAllowlistExemptTargetTest {
  ...
  public void testCreateAllowlistForAUserSucceedsAndSendsCorrectJson() {
    ...
    JsonObject userInnerObject = new JsonObject().add("id", userToAllowlistID).add("type", "user");

    JsonObject userOuterObject = new JsonObject().add("user", userInnerObject);

    String result = TestUtils.getFixture("BoxCollaborationAllowlist/CreateAllowlistForAUser201");

    wireMockRule.stubFor(
        WireMock.post(WireMock.urlPathEqualTo(allowlistURL))
  ...
  public void testGetAllowlistInfoForAUser() {
    ...
    wireMockRule.stubFor(
        WireMock.get(WireMock.urlPathEqualTo(allowlistURL))
            .willReturn(
                WireMock.aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
                    .withBody(result)));
    ...
    BoxCollaborationAllowlistExemptTarget.Info userAllowlistInfo =
        new BoxCollaborationAllowlistExemptTarget(this.api, allowlistID).getInfo();
  ...
  public void testGetAllowlistInfoForAllUsers() {
    ...
    String result =
        TestUtils.getFixture("BoxCollaborationAllowlist/GetAllowlistInfoForAllUsers200");

    wireMockRule.stubFor(
        WireMock.get(WireMock.urlPathEqualTo(allowlistExemptUserURL))
            .willReturn(
                WireMock.aResponse()
                    .withHeader("Content-Type", APPLICATION_JSON)
                    .withBody(result)));
