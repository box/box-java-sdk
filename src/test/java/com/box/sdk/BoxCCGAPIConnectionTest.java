...
public class BoxCCGAPIConnectionTest {
  ...
  public void createsServiceAccountConnection() {
    ...
    BoxAPIConnection api =
        BoxCCGAPIConnection.applicationServiceAccountConnection(
            clientId, clientSecret, enterpriseId);
  ...
  public void createsUserConnection() {
    ...
    api.setRequestInterceptor(
        request -> {
          ...
          assertRequestHeaders(request);
          assertRequestTokenBody(
              request, "some_client_id", "some_client_secret", USER_SUBJECT_TYPE, "some_user_id");
          return new CCGAuthenticationResponse(accessToken, "4245");
        });
  ...
  public void allowsToSaveAndRestoreApplicationConnection() throws URISyntaxException {
    ...
    BoxCCGAPIConnection restoredApi =
        BoxCCGAPIConnection.restore(clientId, clientSecret, savedConnection);
  ...
  public void allowsToSaveAndRestoreUserConnection() {
    ...
    restoredApi.setRequestInterceptor(
        request -> {
          ...
          assertThat(request.getMethod(), is("POST"));
          assertRequestHeaders(request);
          assertRequestTokenBody(request, clientId, clientSecret, USER_SUBJECT_TYPE, userId);
          return new CCGAuthenticationResponse(accessToken, "4245");
        });
  ...
  public void applicationConnectionAllowsToSetAsUserHeader() {
    ...
    api.setRequestInterceptor(
        request -> {
          // then
          Optional<BoxAPIRequest.RequestHeader> header =
  ...
  public void allowsToRevokeToken() {
    ...
    api.setRequestInterceptor(
        request -> {
          ...
          assertThat(request.getMethod(), is("POST"));
          assertRevokeTokenBody(request, clientId, clientSecret, accessToken);
  ...
  private BoxAPIConnection createDefaultApplicationConnection() {
    BoxCCGAPIConnection api =
        BoxCCGAPIConnection.applicationServiceAccountConnection(
            "some_client_id", "some_client_secret", "some_enterprise_id");
  ...
  private void assertRequestTokenBody(
      ...
      String subjectId) {
    List<String> body =
        new BufferedReader(new InputStreamReader(request.getBody(), StandardCharsets.UTF_8))
            .lines()
  ...
  private void assertRevokeTokenBody(
      BoxAPIRequest request, String clientId, String clientSecret, String token) {
    List<String> body =
        new BufferedReader(new InputStreamReader(request.getBody(), StandardCharsets.UTF_8))
            .lines()
            .flatMap(s -> Arrays.stream(s.split("&")))
  ...
  private void assertRequestHeaders(BoxAPIRequest request) {
    Optional<BoxAPIRequest.RequestHeader> header =
        request.getHeaders().stream().filter(h -> h.getKey().equals(CONTENT_TYPE)).findFirst();
    assertThat(header.isPresent(), is(true));
